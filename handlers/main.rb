VOTE_TYPES = {
  ...
  "stop_violations" => {"machine_vote_yes" => "Machine thinks the car stopped at the sign", "machine_vote_no" => "Machine thinks the car did not stop at the sign", "vote_type" => "Car Stop Violation", "vote_negative" => "No Stop", "vote_affirmative" => "Yes Stop", "vote_text" => "Is a car present in this GIF *and* is it stopping at the stop sign? Press J if there's no stop, K if there is a stop (or just click the links below)"},
...
get "/train/:vote_method*" do
  ...
  if @vote_method == "stop_violations" 
    @ssl = StopSignLog.get_random(@vote_method, @previous_stop_id, {"voted_as.presence" => true, "voted_as.full_scene" => true})
  elsif @vote_method == "full_scene"
